...
void ChessPiece::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    if(this == game->selectedPiece) {
        ...
        //check if a king should be blue
        if(this->type() == "KING")
            game->checkForCheck();
    ...
    if(!game->selectedPiece && !this->m_promotionPiece) {
        ...
        game->selectedPiece->currentField()->setBrush(Qt::red);
...
bool ChessPiece::colorAvailableFields(Field *field)
{
    if(!field->isItFree()) {
        ...
        if(k) {
            field->setBrush(Qt::blue);
        } else {
            field->setBrush(Qt::red);
        }
        return true;
    }
    m_possibleMoves.last()->setBrush(Qt::red);
    return false;
}
