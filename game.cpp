...
void Game::displayDeadWhite()
{
    ...
    for(int i = 0, n = whiteDead.size(); i < n; i++) {
        ...
        whiteDead[i]->setPos(8 * board->fieldAt(0,0)->width() + shift * j++, scene->height() - shift*(k+1));
    }
}
...
void Game::displayDeadBlack()
{
    ...
    for(int i = 0, n = blackDead.size(); i < n; i++) {
        ...
        blackDead[i]->setPos(8 * board->fieldAt(0,0)->width() + shift * j++, shift*k);
    }
}
...
{
    //the method is checking if there is a check in the current move.
    // We check the same for black and white king.
    // From the kings field we move diagonally, left, right, up, down and in knight fields
    // and check if there are certain enemy players that
    // attack the king that is being checked
...
void Game::checkForCheckmateOrStalemate()
{
    ...
    //iterate through alive pieces on the board and if there aren't available moves for a side that is on the move
    // then we have checkmate or stalemate - depending on whether the king is checked or not
    bool areTherePossibleMoves = false;
    moveValidationCheck = false;
    //create a copy of the list since generating possible moves modifies 'alivePieces' list if there is eating involved
...
{
    //this method is used for disabling mouse events on the pieces and other buttons when some panel is shown
    //(we don't want a player to be able to move pieces behind panel if a panel is shown)
...
{
    //this method is used to enable mouse events on pieces after a certain panel is destroyed
    //(if some panel is active, player shouldn't be able to move pieces behind it)
...
bool Game::knightAttack(King *a)
{
    ...
    //Checking for possible knight fields
    int i = row - 2;
    int j = col + 1;
...
{
    if(!moves.isEmpty()) {
        // we want to undo move history text, but since we use undo move for
        // checking if a certain move is legal, we don't want to update the history with those checks
        // we want to update history just with regular moves
        if(!moveValidationCheck) {
            QTextCursor cursor = movesHistoryDisplay->textCursor();
            cursor.movePosition(QTextCursor::End);
            if(getTurn() == "BLACK") {
        ...
        //place moved piece to the field before movement
        ...
        //this is a check if a player was in check before he made the move
        ...
        //this is a check if a player has 'eaten' something after he made the move
        ...
        //this is a check if a player made a check to opponent with his move
        ...
        //only time the player can get himself to checked position is during validation
        if(moveValidationCheck) {
